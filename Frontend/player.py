...
class PlayerFrame(ctk.CTkFrame):
    def __init__(self, master, logic_controller, on_rate_callback, on_delete_callback):
        ...
        # Запускаємо таймер один раз при старті
    ...
    def _setup_ui(self):
        ...
        ctk.CTkButton(self.ctrl, text="⏭", width=40, command=self.act_prev, font=("Arial", 16)).pack(side="left", padx=5) # Fix icon font
        self.btn_play = ctk.CTkButton(self.ctrl, text="▶", width=60, command=self.act_play_pause, font=("Arial", 16))
        self.btn_play.pack(side="left", padx=10)
        ctk.CTkButton(self.ctrl, text="⏭", width=40, command=self.act_next, font=("Arial", 16)).pack(side="left", padx=5) # Fix icon font
    ...
    def play_index(self, index):
        if 0 <= index < len(self.playlist):
            ...
            # Логіка, щоб не перезапускати той самий трек, якщо він грає
            ...
            # ВАЖЛИВО: Оновлюємо межі слайдера
            duration = self.current_track[6]
            if duration <= 0: duration = 100 # Захист від ділення на нуль або пустих даних
    ...
    def update_progress(self):
        try:
            ...
            if self.logic.is_playing() and not self.is_dragging and self.current_track:
                ...
                # Авто-переключення (якщо до кінця залишилось менше 1 сек)
            ...
            # Перезапускаємо таймер ЗАВЖДИ, навіть якщо була помилка
